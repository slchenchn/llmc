...
class Quarot(BaseBlockwiseQuantization):
    def __init__(
        self, model, quant_config, input, padding_mask, config, do_preprocess=True
    ):
        super().__init__(model, quant_config, input, padding_mask, config)
        self.special_config = self.quant_config.get("special", {})
        self.dev = torch.device("cuda")
        self.add_quant_config()
        if do_preprocess:
            logger.info("quarot: Preprocess the model")
            self.preprocess()
        else:
    ...
    def preprocess(self):
        ...
        if self.special_config.get("zero_embed", True):
            logger.info("Remove mean from embed")
            self.remove_mean_from_embed()
        else:
            logger.info("Not remove mean from embed")
        ...
        # replace the pre_head_ln with LlmcRMSNorm
        ...
        for rot_layer in self.model.get_extra_rot_module_besides_embed_layers():
            logger.info(
                "For multimodal model, quarot need rotate last layer in projector."
            )
            logger.info(f"rot_layer : {rot_layer}")
            """txt_input     img_input |             | Embeding
            vision_projector |             | | input_embeds | Y.
            ...
            rot_layer.weight.data = torch.matmul(self.Q.T, W).to(
                device="cpu", dtype=dtype
            )  # noqa

            if rot_layer.bias is not None:
                rot_layer.bias.data = torch.matmul(self.Q.T, rot_layer.bias.data).to(
                    device="cpu", dtype=dtype
                )  # noqa

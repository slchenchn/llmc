...
def exchange_row_col(_tensor, i, j):
    """
    Exchanges rows i and j, and then columns i and j of a 2D tensor.
    
    Args:
        _tensor (torch.Tensor): Input 2D tensor to be modified
        i (int): Index of the first row/column to exchange
        j (int): Index of the second row/column to exchange
        
    Returns:
        torch.Tensor: A new tensor with rows i and j exchanged, and columns i and j exchanged
    """
...
def get_rot(n, device='cpu'):
    try:
        if Rot.get(n) is None:
            Rot[n] = pickle.load(open("tools/quant_analysis/Rot.pkl", "rb"))[n]
        R = Rot[n].to(device)

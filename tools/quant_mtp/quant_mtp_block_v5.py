...
def create_random_hadamard_matrix(ori_had_mat):
    ...
    return torch.eye(n, device=ori_had_mat.device, dtype=ori_had_mat.dtype)
    while True:
...
def rotate_mtp_embeddings(state_dict, mtp_pat, rotated_emb, ori_had_mat, new_had_mat):
    ...
    plot_histogram(
        ...
        f"logs/mtp_hists_v5/{mtp_pat}eh_proj_after_first_rotation.png",
    ...
    plot_histogram(
        ...
        f"logs/mtp_hists_v5/{mtp_pat}eh_proj_after_second_rotation.png",
        split_by_second_dim=True,
    )
...
def quant_mtp_block(state_dict, quant_granularity, n_bits):
    ...
    for name in names:
        ...
        if "embed_tokens" in name or "head" in name or "eh_proj" in name:
...
if __name__ == "__main__":
    ...
    save_model_dir = rotated_model_root / "vllm_quant_model_mtp_v5"
    n_bits = 8
    ...
    log_file = "logs/quant_mtp_block_v5.log"
    logger.add(log_file)

...
def act_quant_kernel(x_ptr, y_ptr, s_ptr, BLOCK_SIZE: tl.constexpr):
    """Quantizes the input tensor `x_ptr` and stores the result in `y_ptr` and
    the scaling factor in `s_ptr`.
...
def fp8_gemm_kernel(
    ...
    """Performs a matrix multiplication operation on FP8 matrices with scaling
    factors.

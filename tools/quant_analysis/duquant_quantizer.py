...
import numpy as np
import math
from duquant_utils import get_rot, exchange_row_col, get_hadamard
...
class UniformAffineQuantizer(nn.Module):
    ...
    def rotation(self, weight, max_rotation_step=None, other=None, score_func=None):
        ...
        weight = weight.detach().clone()
        _weight = weight.detach().clone()  # 保存原始权重用于后续恢复
        
        hidden_dim = weight.shape[-1]  # 获取隐藏维度大小
        exchange_ids = []  # 用于记录交换的行/列索引
        peak_values = []  # 用于记录每次旋转后的峰值
